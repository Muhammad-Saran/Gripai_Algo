...
def detect_coin(image):
    ...
    if hand_region:
        ...
        edges = cv2.Canny(blurred, threshold1=150, threshold2=300)

        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (6, 6))
        edges = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, kernel)
        edges_dilated = cv2.dilate(edges, kernel, iterations=1)

        circles = detect_hough_circle_transform(1.2, 50, 100, 30, 30, 200, edges_dilated)
        ...
        if circles is not None:
            ...
            circles = detect_hough_circle_transform(1, 50, 100, 30, 30, 200, edges_dilated)
            if circles is not None:  # Check again after second attempt
                ...
                print("Error: No circles detected in the second attempt.")
                return None
        else:
            print("Error: No circles detected in the first attempt.")
            return None
